...
public class LoggingAspect {
  ...
  public Object loggingMethod(ProceedingJoinPoint pjp) throws Throwable {
    ...
    if (properties.isPrintArgs()) {
      log.atLevel(properties.getLevel()).log("Before -> TimesheetService#{} with arguments: {}", methodName, Arrays.toString(args));
    } else {
    ...
    try {
      ...
      if (properties.isPrintArgs()) {
        log.atLevel(properties.getLevel()).log("After -> TimesheetService#{} with arguments: {}", methodName, Arrays.toString(args));
      } else {
